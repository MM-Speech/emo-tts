...
from emo_tts.infer.utils_infer import transcribe
from emo_tts.model.utils import convert_char_to_pinyin
...
path_data = str(files("emo_tts").joinpath("../../data"))
path_project_ckpts = str(files("emo_tts").joinpath("../../ckpts"))
file_train = str(files("emo_tts").joinpath("train/finetune_cli.py"))
...
def vocab_extend(project_name, symbols, model_type):
    ...
    if model_type == "EmoTTS_v1_Base":
        ckpt_path = str(cached_path("hf://SWivid/Emo-TTS/EmoTTS_v1_Base/model_1250000.safetensors"))
    elif model_type == "EmoTTS_Base":
        ckpt_path = str(cached_path("hf://SWivid/Emo-TTS/EmoTTS_Base/model_1200000.pt"))
...
with gr.Blocks() as app:
    gr.Markdown(
        """
# Emo TTS Automatic Finetune

This is a local web UI for Emo TTS finetuning support. This app supports the following TTS models:
...
For tutorial and updates check here (https://github.com/SWivid/Emo-TTS/discussions/143)
"""
    )
...
```""")
            ...
            exp_name_extend = gr.Radio(
                label="Model", choices=["EmoTTS_v1_Base", "EmoTTS_Base", "E2TTS_Base"], value="EmoTTS_v1_Base"
            )
...
```""")
            with gr.Row():
                exp_name = gr.Radio(label="Model", choices=["EmoTTS_v1_Base", "EmoTTS_Base", "E2TTS_Base"])
